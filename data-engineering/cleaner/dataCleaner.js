...
// ============================================================
// cleanTeams — Scuderie uniche dall'array piloti API
// ============================================================
export function cleanTeams(rawDrivers) {
  ...
  for (const driver of rawDrivers) {
    const teamName = driver?.team?.name
                  ...
                  || driver?.team
                  || null
    const teamNat  = driver?.team?.nationality
    ...
    if (!teamsMap.has(name)) {
      teamsMap.set(name, {
        // Colonne DB: nome, nazionalità_s, punti_totali
        // indice_potenza NON inserito qui — lo imposta l'admin
        nome:           name,
        'nazionalità_s': cleanString(teamNat) || 'N/A',
        punti_totali:   0,   // aggiornato a fine stagione o dall'admin
      })
    }
  }
...
export function cleanDrivers(rawDrivers) {
  ...
  for (const driver of rawDrivers) {
    ...
    if (!firstName || !lastName || !teamName) {
      console.warn(`   [cleaner] Pilota saltato (dati incompleti): ${JSON.stringify(driver).slice(0, 60)}`)
      continue
    }

    cleaned.push({
      // Colonne DB: nome, cognome, nazionalità, numero, id_scuderia_FK (risolto in saveToDb)
      nome:         cleanString(firstName),
      cognome:      cleanString(lastName),
      'nazionalità': cleanString(nat) || 'N/A',
      numero:       parseIntSafe(number) || 99,
      team_nome:    cleanString(teamName),  // usato per lookup id_scuderia_FK
    })
  }
  ...
  for (const race of rawRaces) {
    ...
    if (circKey && !circuitsMap.has(circKey)) {
      circuitsMap.set(circKey, {
        // Colonne DB: nome, paese, lunghezza_tracciato, tipologia
        // indice_imprevedibilità NON inserito qui — lo imposta l'admin
        nome:                cleanString(circKey),
    ...
    races.push({
      // Colonne DB: nome_gara_premio, data, id_circuito_FK (risolto in saveToDb)
      nome_gara_premio: cleanString(raceName)?.slice(0, 30),  // max VARCHAR(30)
      data:             date,
...
// ============================================================
// cleanRaceResults — Risultati e pitstop di una gara
// ============================================================
export function cleanRaceResults({ results, pitStops }, id_gara) {
  ...
  if (Array.isArray(results)) {
    for (const r of results) {
      const pilotaId   = r?.driver?.id     || r?.driverId    || null  // cercheremo per nome/cognome
      ...
      cleanedResults.push({
        // Colonne DB: posizione_arrivo, punti_ottenuti, giro_veloce,
        //             tempo_totale, id_pilota_FK, id_gara_FK
        ...
        punti_ottenuti:   Math.round(parseFloatSafe(points) || 0),  // TINYINT UNSIGNED
        giro_veloce:      fastest ? 1 : 0,
        tempo_totale:     (cleanString(time) || 'N/A').slice(0, 15), // max VARCHAR(15)
      })
    }
  }
  ...
  if (Array.isArray(pitStops)) {
    for (const ps of pitStops) {
      ...
      cleanedPitStops.push({
        // Colonne DB: numero_stop, tempo_pitstop, id_pilota_FK, id_gara_FK
        pilota_nome:    cleanString(firstName),
        pilota_cognome: cleanString(lastName),
        pilota_numero:  parseIntSafe(number),
        id_gara,
...
// Deduce la tipologia del circuito dal nome
